...
io.on('connection', (socket) => {
    ...
    // ────────────────────────────────────────────────
    socket.on('signal', (to, _from, data) => {
    ...
    socket.on('join-room', async (data) => {
        ...
        try {
            ...
            const currentUsersList = Array.from(activeUsers.get(roomId), ([socketId, user]) => ({ ...user, socketId }));
    ...
    socket.on('send-message', async (data) => {
        ...
        if (!roomId || !senderUid || !senderUsername || !text?.trim())
            return;
        try {
